...
def _read_original_qqp(filename):
    ...
    fieldnames = ["id", "qid1", "qid2",
                  "question1", "question2", "is_duplicate"]
    csvin = open(filename, "r", encoding="utf8")
...
def _build_and_write_row(row, writer, qid_to_words):
    ...
    sentence1 = _build_sentence(
        row["sentence1"], int(row["qid1"]), qid_to_words)
    if not sentence1:
        return False
    sentence2 = _build_sentence(
        row["sentence2"], int(row["qid2"]), qid_to_words)
    if not sentence2:
        return False
    else:
...
def main(args):
    ...
    paws_qqp_output_csv = open(args.paws_output, "w", encoding="utf8")
    fieldnames = ["id", "sentence1", "sentence2", "label"]
    writer = csv.writer(paws_qqp_output_csv, delimiter="\t", quotechar=None)
    writer.writerow(fieldnames)

    paws_qqp_input_csv = open(args.paws_input, "r", encoding="utf8")
